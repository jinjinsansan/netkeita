...
async def call_backend(endpoint: str, payload: dict) -> dict:
    """Call Dlogic backend API."""
    url = f"{DLOGIC_API_URL}{endpoint}"
    try:
        resp = _client.post(url, json=payload, timeout=60)
        resp.raise_for_status()
        return resp.json()
    except Exception:
        logger.exception(f"Backend API call failed: {endpoint}")
        return {}


def get_predictions(race_data: dict) -> dict:
    """Call predictions API. Returns full-scores per horse."""
    payload = {
